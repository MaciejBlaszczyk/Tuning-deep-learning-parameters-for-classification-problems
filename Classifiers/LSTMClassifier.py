...
from Utilities.Utilities import write_to_file, count_words
...
class LSTMClassifier(Classifier):
    def __init__(self, data_handler, batch_size, epochs, eval_type, hyperparams):
        ...
        self.embedded_vector_lenghts = hyperparams['embedded_vec_len']
        self.cells = hyperparams['cells']
        self.vocabulary = count_words(self.data_handler.X_scaled)
        self.results_path = os.path.dirname(os.path.abspath(__file__)) + '/LSTM_' + data_handler.dataset_name[
                                                                                    :-7] + '_Results.tsv'

    def create_model(self, hidden_layers, cells, embedded_vec_len):
        model = Sequential()
        if hidden_layers == 1:
            model.add(LSTM(cells, input_shape=(self.data_handler.features, 1)))
        else:
            for _ in range(hidden_layers - 1):
                model.add(LSTM(cells, return_sequences=True, input_shape=(self.data_handler.features, 1)))
    ...
    def perform_classification(self):
        ...
        for hidden_layers in self.hidden_layers:
            for embedded_vec_len in self.embedded_vector_lenghts:
                for cell in self.cells:
                    ...
                    for _ in range(repeats):
                        for train_index, test_index in self.data_handler.skf.split(self.data_handler.X_scaled,
                            ...
                            model = self.create_model(hidden_layers, cell, embedded_vec_len)
                    ...
                    write_to_file(self.results_path,
                                  ...
                                  ClassifierType.LSTM.name,
                                  self.eval_type.name,
                                  str(hidden_layers),
                                  str(cell),
                                  str(embedded_vec_len),
